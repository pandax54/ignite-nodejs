...
let CreateCarUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    // carros com a mesma placa nao podem ser cadastrados duas vezes
    const checkLicensePlate = await this.carsRepository.findByLicensePlate(license_plate);

    if (checkLicensePlate) {
      throw new _AppError.AppError("Car already registered!");
    }
